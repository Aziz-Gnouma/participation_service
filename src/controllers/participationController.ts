...
export const createParticipation = async (req: Request, res: Response) => {
  try {
    const { clientID, carpoolingID, etat } = req.query as unknown as ParticipationDTO;
...
export const getParticipationsByClientId = async (req: Request, res: Response) => {
  const clientID = req.params.id;

  try {
    console.log('Attempting to find participations for client with ID:', clientID);

    const participations = await Participation.find({ clientID }).lean().exec();
    ...
    const participationDTOs = participations.map(participation => mapParticipationToDTO(participation));
